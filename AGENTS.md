# AGENTS.md — cronologia/aparecida
...
A compiled static website documenting the chronology of **Nossa Senhora
Aparecida** — the image reportedly netted from the Paraíba do Sul in 1717,
the devotion, and the acts that honoured it. A single JSON file is the source
of truth; a zero-dependency Node script compiles it into static HTML served
by GitHub Pages.
...
The five core rules of the sourcing-rules skill apply verbatim in spirit:
cite it or flag it; attribute, don't assert; sources span the spectrum by
design; time-sensitive statuses are dated; testimony is a perspective, not a
fact source. On top of them, this repo's own rules:

1. **Honours are not findings — and this repo has only honours.** There is
   no apparition and no discernment judgment anywhere in this case: what was
   found in 1717 was a statue, and every Church act in the chronology
   (coronation 1904, basilica titles 1908/1980, patronage 1930, golden roses
   1967/2007/2017, consecrations 1931/1980) honours or regulates a devotion.
   Record each act as an act about a named object — an image, a church, a
   nation's patronage — and never sum them into a verdict. **This repo
   deliberately declares no `approvalLadder`**: there is no discernment case
   to ladder, and adding one would manufacture the very misreading the
   dataset exists to prevent (core ADR-0007; the DDF's 2024 *Norms* govern
   alleged supernatural phenomena, which a netted statue is not —
   `ddf-norms-2024`).
2. **Carry the attestation gap everywhere the date appears.** The earliest
   surviving account is from August 1757 (~40 years after the event) and
   says "1719, pouco mais ou menos"; 1717 is a historians' inference from
   the Count of Assumar's itinerary; no day is attested (12 October is a
   20th-century convention). The 1717 event's `dateNote` is the canonical
   statement; do not let new prose assert "12 October 1717" in site voice.
3. **A12 is the devotion's official voice, not corroboration.** Anything
   resting on the Santuário's media operation alone (the 1901 petition
   details, the 1903 Chapter grant, the 1967 rose logistics, the
   foundation-stone story) stays labeled official-devotional and flagged.
4. **Record the tangles, don't resolve them**: 1743-vs-1757 earliest-text
   attribution; 1983-vs-1984 National Shrine declaration; the 1978 return
   date. Each carries a `dateNote` naming the disagreement.
5. **The 1930 decree text is offline** (Aparecida curia archive); cite the
   date and content to the academic account, and keep the AAS 22 (1930)
   library check open rather than linking a paraphrase as the act.
